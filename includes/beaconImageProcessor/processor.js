...
class SpectralClassDetector {
    static detect(img) {
        ...
        for (const spectralClass in BGRSpectralClasses) {
            filteredImgs[spectralClass] = img.bitwiseAnd(img.inRange(BGRSpectralClasses[spectralClass], BGRSpectralClasses[spectralClass]).cvtColor(opencv4nodejs_prebuilt_1.default.COLOR_GRAY2BGR));
        }

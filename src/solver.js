...
class Solver {
    static async solve(beaconImg) {
        const img = opencv4nodejs_1.default.imdecode(beaconImg);
        ...
        if (potentialMatchingSystems.length <= 1)
            ...
            const promises = potentialMatchingSystems.map(async (systemName) => {
                let systemImg = opencv4nodejs_1.default.imdecode(await fs_1.default.promises.readFile(`./beaconImages/${systemName}.png`));
                systemImg = systemImg.copyMakeBorder(200, 200, 200, 200, opencv4nodejs_1.default.BORDER_CONSTANT, new opencv4nodejs_1.default.Vec3(0, 0, 0));
                return {
                    name: systemName,
                    confidence: opencv4nodejs_1.default.minMaxLoc(systemImg.matchTemplate(img, opencv4nodejs_1.default.TM_CCOEFF)).maxVal
                };
            });
